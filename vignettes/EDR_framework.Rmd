...
# Define a function applying the Lotka-Volterra model
LVmod <- function(t, x, parms){
  with(as.list(c(x, parms)), {
    dx1_dt = r1*x1 * (1 - a11*x1 - a12*x2)
    dx2_dt = r2*x2 * (1 - a22*x2 - a21*x1)
    list(c(dx1_dt, dx2_dt))
  })
}

# We can use deSolve to run the simulations
library(deSolve)
simulated_abun <- lapply(1:nrow(initial), function(isampling){
  # Initial abundance in the sampling unit i
  x <- c(x1 = initial$sp1[isampling], x2 = initial$sp2[isampling])
  # Simulate community dynamics
  simulated_abun <- data.frame(ode(y = x, times = ID_obs, func = LVmod, parms = parms))
...
# Define a function applying the Lotka-Volterra model for three species
LVmod_3sp <- function(t, x, parms){
  with(as.list(c(x, parms)), {
    dx1_dt = r1*x1 * (1 - a11*x1 - a12*x2 - a13*x3)
    dx2_dt = r2*x2 * (1 - a22*x2 - a21*x1 - a23*x3)
    dx3_dt = r3*x3 * (1 - a33*x3 - a31*x1 - a32*x2)
    list(c(dx1_dt, dx2_dt, dx3_dt))
  })
}
...
simulated_abun2 <- lapply(1:nrow(initial), function(isampling){
  # Initial abundance in the sampling unit i
  x <- c(x1 = initial$sp1[isampling], x2 = initial$sp2[isampling], x3 = initial$sp3[isampling])
  # Simulate community dynamics
  simulated_abun <- data.frame(ode(y = x, times = ID_obs, func = LVmod_3sp, parms = parms2))
  # Add the names of the sampling units
...
simulated_abun3 <- lapply(1:nrow(initial), function(isampling){
  # Initial abundance in the sampling unit i
  x <- c(x1 = initial3$sp4[isampling], x2 = initial$sp2[isampling])
  # Simulate community dynamics
  simulated_abun <- data.frame(ode(y = x, times = ID_obs, func = LVmod, parms = parms3))
  # Add the names of the sampling units
