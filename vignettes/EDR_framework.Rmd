...
An **Ecological Dynamic Regime (EDR)** is defined as the *"natural fluctuations of ecosystem states around some trend or average resulting from an intricate mix of internal processes and external forces that push the system towards specific domains of attraction"* (Sánchez-Pinillos et al.).

EDRs are composed of a set of ecological trajectories showing similar processes in the development, interaction, and reorganization of some state variables over time. Depending on the state variables, ecological trajectories are usually defined in multivariate spaces that make challenging the characterization and comparison of EDRs.

The **EDR framework** is a set of algorithms and metrics aiming to characterize and compare EDRs composed of ecological trajectories defined in multidimensional state spaces. The EDR framework is implemented in `ecoregime`. You can find the formal definition of the EDR concept, all the methodological details of the framework, and some illustrative examples with artificial and real data sets in the following publication:
...
The essential unit of an EDR is the **ecological trajectory**. An ecological trajectory is defined as a sequence of observations capturing the temporal changes of some state variables. You can probably skip this section if you are familiar with ecological trajectories defined in multivariate spaces. Otherwise, I recommend you to read on.

Let me define a simple example that I will use in this vignette. Imagine that we have 10 sampling units in which we are monitoring the temporal changes in some state variables. For example, those sampling units (*A, B, ..., J*) could be permanent plots or transects in which we have inventoried the abundance of two species (*sp1, sp2*) at three temporal points (*t* = 1, *t* = 2, *t* = 3). 
...
In our example, we can use `vegan` to calculate the Bray-Curtis dissimilarity and generate **$D_O$**.
...
We can visualize the state space using ordination methods. Here, I apply multidimensional scaling (mMDS; `smacof`) to the state dissimilarity matrix (**$D_O$**) and plot the distribution of the ecological states in that ordination space. We can use different colors and symbols to represent the observations of each sampling unit:
...
plot(state_mds$D1, state_mds$D2,
     ...
     pch = rep(1:10, each = 3), # use different symbols for each sampling unit
...
## -->> This code shows you the process step by step, but you could directly use 
## -->> the function trajectoryPlot() in ecotraj
...
plot(traj_mds$D1, traj_mds$D2,
     ...
     pch = 1:10, # use different symbols for each sampling unit
     xlab = "Axis 1", ylab = "Axis 2",
...
for (isampling in 1:10) {
  points(abundance[which(abundance$sampling_unit == LETTERS[isampling]), ]$time,
         abundance[which(abundance$sampling_unit == LETTERS[isampling]), ]$sp1,
         col = traj.colors[isampling], pch = isampling)
  lines(abundance[which(abundance$sampling_unit == LETTERS[isampling]), ]$time,
         abundance[which(abundance$sampling_unit == LETTERS[isampling]), ]$sp1,
         col = "black", pch = isampling)
  
  points(abundance[which(abundance$sampling_unit == LETTERS[isampling]), ]$time,
         abundance[which(abundance$sampling_unit == LETTERS[isampling]), ]$sp2,
         col = traj.colors[isampling], pch = isampling)
  lines(abundance[which(abundance$sampling_unit == LETTERS[isampling]), ]$time,
         abundance[which(abundance$sampling_unit == LETTERS[isampling]), ]$sp2,
         col = "red", pch = isampling, lty = 2)
}
...
These analyses are fine for extremely simple systems as the ones simulated in our toy dataset. However, when we want to apply these or similar approaches to empirical data, the story is very different. In real ecosystems, we usually have to deal with a great heterogeneity, multiple variables interacting between each other, and noise. As a consequence, we need robust tools able to summarize a great variety of trajectories in multidimensional spaces. That is precisely what RETRA-EDR pursues.

### 4.1. Identifying representative trajectories with RETRA-EDR

**RETRA-EDR (REpresentative TRAjectories in Ecological Dynamic Regimes)** is an algorithm that aims to identify representative trajectories in EDRs based on the distribution and density of their ecological trajectories (Sánchez-Pinillos et al.). Whereas many algorithms with similar goals where developed for moving objects in low-dimensional spaces, RETRA-EDR can be applied to high-dimensional spaces, capturing the complexity of empirical data.

I will not explain the details of RETRA-EDR in this vignette (you can find them in Sánchez-Pinillos et al.), but it is important that you know its main steps to understand the output returned by `retra_edr()`:
...
Let's apply `retra_edr()` to our data. As the EDR is composed of only 10 trajectories, we will assign a value of `minSegs` relatively low.
...
```{R RETRA-EDR_segs}
# Calculate segment dissimilarity: retra_edr() uses segmentDistance() in ecotraj
...
We obtain two representative trajectories whose attributes can be summarized using the function `summary()`: 

```{r retra_summary}
summary(repr_traj)
```

`retra_edr()` returns an object of class `RETRA`. We can extract specific attributes as we do with lists. 
...
- *T2* is composed of the three states of the sampling unit *C*, converging to *T1* through a link between the last state of *C* and the second state of *I*.
...
# Add a legend for the representative trajectories
...
In this example, the lengths of the artificial links are very small. 

```{r retra_links}
lapply(repr_traj, "[[", "Link_distance")
```

In more complex cases, you could want to discard some segments or split representative trajectories based on long artificial links. For example, you could split *T1* and *T2* to separate the last segment (`"E[2-3]"`). For that, you can use the function `define_retra()`.
...
new_traj <- data.frame(
  ...
  RT = c(rep("NT1", 4), rep("NT2", 5), rep("NT3", 2)),
  # name of the trajectories (sampling units)
  RT_traj = c(rep("B", 2), rep("I", 2), # for the first trajectory (NT1)
              rep("C", 3), rep("I", 2), # for the second trajectory (NT2)
              rep("E", 2)), # for the third trajectory (NT3)
  # states in each sampling unit 
  RT_states = c(1:2, 2:3, # for the first trajectory (NT1)
                1:3, 2:3, # for the second trajectory (NT2)
                2:3), # for the third trajectory (NT3)
  # representative trajectories obtained in retra_edr()
  RT_retra = c(rep("T1", 4), rep("T2", 5), 
               rep("T2", 2)) # The last segment belong to both (T1, T2), choose one
)

new_repr_traj <- define_retra(data = new_traj, 
...
plot(new_repr_traj, # <-- This is the RETRA object returned by define_retra()
     ...
     # display representative trajectories in blue    
     RT.colors = "blue",
     # select NT2 to be displayed in a different color (black)
     select_RT = "NT2", sel.color = "black",
     # Identify artificial links using dashed lines (default) and a different color (red)
     link.lty = 2, link.color = "red",
     # We can use other arguments in plot()
...
# Add a legend for the representative trajectories
legend("bottomright", c("NT1, NT3", "NT2", "Link"), 
       col = c("blue", "black", "red"), lty = c(1, 1, 2))
...
Let's see how the species diversity varies along the representative trajectories returned by `retra_edr()`. First, we need to calculate an index of species diversity (e.g., Shannon index) for the states in each representative trajectory and set the order of the states in the trajectory:
...
Then, we can evaluate the variation of the species diversity along the representative trajectories graphically:
...
## 5. Variability and heterogeneity of ecological trajectories in EDRs
...
**Dynamic dispersion (dDis)** is calculated as the average dissimilarity between the trajectories in an EDR and another trajectory taken as a reference (Sánchez-Pinillos et al.).
...
# Add representative trajectories' data to abundance
abundance_T2 <- rbind(abundance, abundance_T2[, names(abundance)])

# Calculate state dissimilarities
state_dissim_T2 <- vegan::vegdist(abundance_T2[, c("sp1", "sp2")], method = "bray")

```

Then, we can compute dDis using the function `dDis_edr()`:

```{r dDis_T2}
dDis_T2 <- dDis_edr(d = state_dissim_T2, d.type = "dStates", 
...
Alternatively, you can weight each trajectory depending on some criteria. Currently, `dDis_edr()` is able to compute *dDis* weighting ecological trajectories by their size (`w.type = "size"`) and length (`w.type = "length"`). However, if you want to use different criteria, you can use a set of weights of your choice (`w.type = "precomputed"`) and indicate them through the `w.values` argument. For example, imagine that you want to weight each trajectory by the initial abundance of *sp2*:

```{r dDis_w}
# Define w values
w.values <- abundance[which(abundance$time == 1), ]$sp2
...
# Considering C as reference
dDis_I_w <- dDis_edr(d = traj_dissim, d.type = "dTraj", 
         trajectories = ID_sampling, reference = "I", 
         w.type = "precomputed", w.values = w.values[-ind_I])
# Considering C as reference
dDis_F_w <- dDis_edr(d = traj_dissim, d.type = "dTraj", 
         trajectories = ID_sampling, reference = "F", 
         w.type = "precomputed", w.values = w.values[-ind_F])
dDis_I_w
dDis_F_w
...
Weighting ecological trajectories is equivalent to modifying the location of the trajectory taken as the reference. Therefore, I do not recommend you to weight trajectories if the representative trajectory is the reference of *dDis since it could make difficult the interpretation of the results.
...
*dBD* is implemented in the function `dBD_edr()`. As you can imagine, *dBD* in our EDR is very small since all trajectories follow the same dynamical patterns. 
```{r dBD}
dBD_edr(d = state_dissim, d.type = "dStates", 
...
**Dynamic evenness (dEve)** quantifies the regularity with which an EDR is filled by ecological trajectories (Sánchez-Pinillos et al.). This metric informs about the existence of groups of trajectories within the EDR.

You can compute *dEve* with the function `dEve_edr()`:

```{r dEve}
dEve_edr(d = traj_dissim, d.type = "dTraj", trajectories = ID_sampling)
...
For example, weighting ecological trajectories by the initial abundance of *sp2* is equivalent to reducing the dissimilarity of the trajectories dominated by *sp2*.
...
We will use the initial states of our first EDR for *sp1* and *sp2*, but we will see what happens when we introduce a third species (*sp3*).
...
parms2 <- c(r1 = 1, r2 = 0.1, a11 = 0.02, a21 = 0.03, a22 = 0.02, a12 = 0.01,
           r3 = 1.2, a33 = 0.02, a31 = 0.01, a32 = 0.01, a13 = 0.1, a23 = 0.1)
...
The third EDR will be composed of trajectories dominated by two new species (*sp4*, *sp5*) able to coexist in equilibrium:
...
initial3 <- data.frame(sampling_units = ID_sampling3, 
                   ...
                   sp5 = round(runif(10), 2))

# Define the parameters for the Lotka-Volterra model
parms3 <- c(r1 = 1, r2 = 1, a11 = 0.2, a21 = 0.1, a22 = 0.02, a12 = 0.01)

# We can use primer to run the simulations
simulated_abun3 <- lapply(1:nrow(initial), function(isampling){
  # Initial abundance in the sampling unit i
  initialN <- c(initial3$sp4[isampling], initial3$sp5[isampling])
  # Simulate community dynamics
  ...
  simulated_abun$sp5 <- simulated_abun$X2/rowSums(simulated_abun[, c("X1", "X2")])

  return(simulated_abun[, c("sampling_unit", "time", "sp4", "sp5")])
})
...
state_dissim_allEDR <- vegan::vegdist(abundance_allEDR[, paste0("sp", 1:5)], 
                                      method = "bray")
...
If we plot the trajectories of all EDRs in a common trajectory space, we can see that all trajectories of *EDR3* are very far from those in *EDR1* and *EDR2.* This is not surprising since *EDR3* does not share any species with *EDR1* and *EDR2.*

```{r traj_mds_allEDR, fig = TRUE, fig.height = 5, fig.width = 5, fig.align = "center"}
# Multidimensional scaling
traj_mds_all <- smacof::smacofSym(traj_dissim_allEDR, 
                                  ndim = nrow(as.matrix(traj_dissim_allEDR))-1)
traj_mds_all <- data.frame(traj_mds_all$conf)

# Plot the distribution of the states in the state space
traj.colorsEDRs <- RColorBrewer::brewer.pal(3, "Set1")
plot(traj_mds_all$D1, traj_mds_all$D2,
     col = rep(traj.colorsEDRs, each = 10), # use different colors for each EDR
     pch = rep(15:17, each = 10), # use different symbols for each EDR
     xlab = "Axis 1", ylab = "Axis 2",
     main = "EDRs in the trajectory space")
legend("bottomright", paste0("EDR", 1:3), col = traj.colorsEDRs, pch = 15:17)
...
EDR_dissim <- dist_edr(dTraj = traj_dissim_allEDR, edr = rep(c("EDR1", "EDR2", "EDR3"), each = 10), 
                       metric = "DDR")
round(EDR_dissim, 2)
